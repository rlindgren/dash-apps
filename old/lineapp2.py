...
# NWT -- MONTHLY DECADAL TEMPERATURE AVERAGES Application

# # APP INPUT DATA
df = pd.read_csv( './data/tas_minesites_decadal_monthly_mean_alldata_melted.csv', index_col=0 )
                    ...
                    html.Div([
                        ...
                        html.Div([
                            ...
                            html.Div( [html.Label('Choose Month(s)', style={'font-weight':'bold'}),
                                    ...
                                    ], id='month-div', className='four columns' )
                            ], className='row'),
...
def average_months( dff, model, scenario ):
    dfm = dff[(dff['model'] == model) & (dff['scenario'] == scenario)].groupby( 'month' )\
            .apply(lambda x: x['tas']\
            .reset_index(drop=True))\
            .T\
            .mean(axis=1)\
    # convert back to a DataFrame from the output Series...
    dfm = dfm.to_frame(name='tas').reset_index(drop=True)
    dfm['year'] = dff['year'].unique()
    dfm['model'] = model
...
def prep_data( minesite, year_range, scenario_values, model_values, months ):
    import itertools
    print( 'prepping data' )
    print( months )
    ...
    if len(dff.month.unique()) > 1:
        print(dff.model.unique())
        print(dff.scenario.unique())
        dff = pd.concat([ average_months( dff, m, s ) for m,s in itertools.product(dff.model.unique(), dff.scenario.unique()) ])

        # for model in dff.model.unique():
        #     for scenario in dff.scenario.unique():
        #         dfm = dff[(dff['model'] == model) & (dff['scenario'] == scenario)]
        #                 .groupby( 'month' )
        #                 .apply(lambda x: x['tas']
        #                 .reset_index(drop=True))
        #                 .T
